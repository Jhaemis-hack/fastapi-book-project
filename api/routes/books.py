...
async def create_book(book: Book) -> JSONResponse:
    db.add_book(book)
    return JSONResponse(
...
async def get_books() -> list[Book]:
    return list(db.get_books().values())  # ✅ Convert OrderedDict values to list
...
async def get_book(book_id: int) -> Book:
    ...
    if book is None:
        raise HTTPException(status_code=404, detail="Book not found")  # ✅ Handle missing book
    return book
